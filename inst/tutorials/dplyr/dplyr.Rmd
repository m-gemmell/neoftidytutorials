...
The third, and final, code:

-   Keeps R2 `transect` rows
-   Arranges the rows by `stem_length`
-   Selects the 6th, 9th, and 10th columns
-   Slices out the bottom 8 rows
...
<center>![](./images/dplyr_col_names.png){style="width:150px"}</center>

As **tibbles** do not have row names (see [Tibble structures](https://neof-workshops.github.io/Tidyverse/tibble/structure.html)), column/variable names are frequently used in functions across `tidyverse` packages.

### Column names without quotes

For ease of use, column/variable names can be used without quotes in `tidyverse` functions.

The below code is extracting the column `species` as a vector with the [`pull()`](/dplyr/pull.qmd) function.

```{r mammal_species_pull, echo=TRUE}
mammal_sleep_tbl |>
  #Slice out top 10 rows
  dplyr::slice_head(n=10) |>
  #Pull out species column as a vector
  dplyr::pull(species)
```

Compare this to base R which requires quotes around the column name.

```{r mammal_species_base_r, echo=TRUE}
mammal_sleep_tbl[1:10,"species"] |> as.vector()
```

### Column names with quotes

However, quotes are needed for certain types of column/variable names.
These include:

-   Column names with spaces (e.g. `Average Height`)
-   Column names with special characters such as `/` (e.g `Territory/Region`)
-   Column names that are numbers (e.g. `2020`)

Although double or single quotes can be used we suggest using the back tick `` ` ``.
It is normally the top left key below the esc key.

The below code is extracting the column `Average Index` as a vector with the [`pull()`](/dplyr/pull.qmd) function.

```{r living_plant_pull, echo=TRUE}
living_planet_tbl |>
  #Slice out top 10 rows
  dplyr::slice_head(n=10) |>
  #Pull out species column as a vector
  dplyr::pull(`Average Index`)
```

### To quote or not to quote

Thankfully when printing or [glimpsing](https://neof-workshops.github.io/Tidyverse/dplyr/glimpse.html) (covered later) a **tibble** it will show which columns need quotes.

Notice the `` `Shucked Weight` ``, `` `Viscera Weight` ``, and `` `Shell Weight` `` column names with their back ticks in the below code output.
They will need bacticking/quoting in `tidyverse` functions.

```{r crab_age_print, echo=TRUE}
crab_age_pred_tbl |> print()
```

```{r crab_age_glimpse, echo=TRUE}
crab_age_pred_tbl |> glimpse()
```

## Row functions

<center>![](./images/dplyr_rows.png){style="width:150px"}</center>

### The four verbs

The four main `dplyr` verbs (i.e. functions) to manipulate rows are:

-   [**`arrange()`**](https://neof-workshops.github.io/Tidyverse/dplyr/arrange.html): Arrange the rows of a **tibble**
...
-   [**`filter()`**](https://neof-workshops.github.io/Tidyverse/dplyr/filter.html): Extract rows by filtering with conditions
    -   Can be used to pick rows of certain groups, filter based on numeric sizes, and more
-   [**`slice()`**](https://neof-workshops.github.io/Tidyverse/dplyr/slice.html): A set of methods to choose a slice of rows based on index positions, top and bottom observations, and min and max values based on a specific column
...
Rather than challenges please look at the quick examples.
...
hbr_maples |>
  #Filter to keep leaf_dry_mass smaller or equal to 0.015
...
<center>![](./images/dplyr_columns.png){style="width:150px"}</center>

### The six verbs

The six main verbs (i.e. functions) to manipulate columns are:

-   [**`glimpse()`**](https://neof-workshops.github.io/Tidyverse/dplyr/glimpse.html): Print a **tibble** in a transposed manner
    -   Useful for seeing the data types of all the columns.
...
-   [**`relocate()`**](https://neof-workshops.github.io/Tidyverse/dplyr/relocate.html): Relocate columns
    -   You can relocate columns to the start or end, and you can move them after or before specified columns
-   [**`rename()`**](https://neof-workshops.github.io/Tidyverse/dplyr/rename.html): Rename columns in a **tibble**
-   [**`select()`**](https://neof-workshops.github.io/Tidyverse/dplyr/select.html): Select specific columns of a **tibble**
...
[Dplyr glimpse() page](https://neof-workshops.github.io/Tidyverse/dplyr/glimpse.html)
...
Subset the **tibble** `crab_age_pred_tbl` to only retain the `Diameter` column.
...
mammal_sleep_tbl |>
  #Arrange by life span (descending)
...
Relocate the `utm_easting` column to after the `utm_northing` column in `nwt_pikas`.
...
Move the date class columns to the end of `nt_icecover`.
...
Grouped **tibbles** can then be analysed with the following functions:

-   `count()`: Count the number of observations per group in a grouped **tibble**
-   `summarise()`: Summarise values within a grouped **tibble**
...
You will need various `dplyr` functions that you have learnt throughout this tutorial.
...
Carry out the following tasks with the created `mammal_sleep_tbl`:
...
1.  Create a **tibble** called `mammal_wt_tbl` only containing the rows for the top 10 dreamers (i.e. top 10 highest `dreaming` values).
    -   Ensure the **tibble** only contains the columns "species", "body_wt", and "brain_wt".
2.  Create a **tibble** called `mammal_predation_tbl` only containing the rows for the top 10 dreamers (i.e. top 10 highest `dreaming` values).
    -   Ensure the **tibble** only contains the columns "species", "predation", "exposure" and "danger".
...
Excellent work on completing the `dplyr` tutorial.
You've gone through pipes, row and column functions, grouping, and binding of tibbles.
It is a lot but the main website will act as a "cookbook" for your future reference. I find it hard myself to remember the names of the functions and how they all work so I constantly refer to it as I carry out my R coding.

The next tutorial will be a much smaller one focussing on `tidyr` which will me incredibly useful for the `ggplot2` tutorial.
